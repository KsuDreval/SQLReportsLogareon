...
(
	...
	where sd."SKUName" != 'Предопределенный ОХ' and sd."LocationParentUUID" != ''
...
(
	...
	from stockdatamart sd
	left join sku_lwh sl on sl."uuid" = sd."SKUUUID"
	where sd."SKUName" != 'Предопределенный ОХ' and sd."LocationParentUUID" = '' and sd."ZoneName" != 'МХ без зоны'
...
select
	...
	, zs.zone_max_volume
	, case 
		when (zs.zone_max_volume = 0 or zs.zone_volume is null) then null
	...
	, zs.zone_max_weight
	, case 
		when (zs.zone_max_weight = 0 or zs.zone_weight is null) then null
